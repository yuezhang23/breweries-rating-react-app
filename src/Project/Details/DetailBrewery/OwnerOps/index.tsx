...
const toggleStoreModal = () => setShowStoreModal(!showStoreModal);
  ...
  const handleUpdateBeer = async () => {
    try {
      ...
      const updatedBeer = await detailClient.updateBeer(beer._id, beer);
  ...
  const selectBeer = async (beer: Beer) => {
    try {
      const b = await detailClient.findBeerById(beer._id);
      setBeer(b);
    } catch (error) {
      console.error("Failed to select beer", error);
    }
  };
  ...
  const handleUpdateStore = async () => {
    try {
      ...
      const updatedStore = await detailClient.updateStore(store._id, store);
